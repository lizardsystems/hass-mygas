...
- **device_id** - Устройство Прибор учета
- **date** - Дата, за которую запрашивается счет, по умолчанию - первый день текущего месяца. Необходимо указывать первый день месяца, за который запрашивается счет. Например, для получения счета за январь 2024 года необходимо указать `2024-01-01`. В случае, если дата не указана, то будет запрашиваться счет за текущий месяц. Если указан не первый день месяца, то сервис не возвращает данные.
...
data:
  ...
  date: 2024-01-01
```
**Обязательно указывать дату в формате YYYY-MM-DD, например 2024-01-01 и принимается только первый день месяца**
...
Интеграция `mygass` поддерживает сервис `mygass.get_bill`, который принимает параметры `device_id` и `date`. Для удобства можно создать пользовательский интерфейс с выбором даты и кнопкой вызова сервиса.
